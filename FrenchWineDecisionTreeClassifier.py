...
plt.figure(figsize=(20, 10)) #利用seaborn库绘制参数图
...
a=input("请输入第一组数据:").split(',')
b=input("请输入第二组数据:").split(',')
c=input("请输入第三组数据:").split(',')
X_new=[a,b,c]
print(X_new)#利用3个数据进行测试，即取3个数据作为模型的输入端
model.predict(X_new)
print(model.predict(X_new))#输出测试的结果，即输出模型预测的结果
